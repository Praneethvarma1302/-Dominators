...
def mainroot(login_user_id):
	print(login_user_id)
	# check if login user is super admin or not
	cur.execute("Select * from user where id = ?",(login_user_id,))
	x = cur.fetchone()
	isSuperAdmin = True if x[6] else False
	...
	def connectprinter():
		os.startfile("D:/TestFile.pdf", "print")
	#---------------top frame------------------------------------------------------------------------------------------------------------------
	...
	#-----------------------------------------------------------------
		nl = Label(b_frame,text='Made by Dominators',fg='black',bg='gray91',font='msserif 8')
		nl.place(x=955,y=310)
		nl.tkraise()
	#-----------------------------Profile----------------------------------------------------
	def profile_information():
		b_frame = Frame(root,height=400,width=1080,bg='white')
		Label(b_frame,text='Personal Information',font='msserif 20',bg='gray93').place(x=450,y=0)
		...
		Label(b_frame,text='Name :',bg='gray93',font='15').place(x=330,y=50)
		name = Entry(b_frame, font='msserif 14')
		name.insert(0, x[1])
		name.place(x=430,y=50)
		name.configure(state='readonly')
		

		Label(b_frame,text='Email :',bg='gray93',font='15').place(x=330,y=80)
		email = Entry(b_frame, font='msserif 14')
		email.insert(0, x[2])
		email.place(x=430,y=80)
		email.configure(state='readonly')


		Label(b_frame,text='Phn No :',bg='gray93',font='15').place(x=330,y=110)
		phn = Entry(b_frame, font='msserif 14')
		phn.insert(0, x[3])
		phn.place(x=430,y=110)
		phn.configure(state='readonly')

		Label(b_frame,text='Gender :',bg='gray93',font='15').place(x=330,y=140)
		var = StringVar()
		var.set(str(x[4]))
		male_rb = Radiobutton(b_frame, text='Male',bg='#CCCCCC',variable=var,value='male',font=('Times', 10),width=4)
		male_rb.place(x=430,y=140)
		female_rb = Radiobutton(b_frame,text='Female',bg='#CCCCCC',variable=var,value='female',font=('Times', 10),width=4)
		female_rb.place(x=510,y=140)
		others_rb = Radiobutton(b_frame,text='Others',bg='#CCCCCC',variable=var,value='others',font=('Times', 10),width=4)
		others_rb.place(x=580,y=140)
		
		male_rb.configure(state='disabled')
		female_rb.configure(state='disabled')
		others_rb.configure(state='disabled')
		def editDetails():
			# Make fields editable
			if editDetails_btn.config('relief')[-1] == 'sunken':
				editDetails_btn.config(text="Save")
				editDetails_btn.config(relief="raised")
				male_rb.configure(state='normal')
				female_rb.configure(state='normal')
				others_rb.configure(state='normal')
				phn.configure(state='normal')
				email.configure(state='normal')
				name.configure(state='normal')
			else:
				print(name.get(),email.get(),phn.get(),var.get(),id)
				cur.execute("update user set name=?,email=?,contact=?,gender=? where id=?",(name.get(),email.get(),phn.get(),var.get(),id))
				con.commit()
				editDetails_btn.config(text="Edit")
				editDetails_btn.config(relief="sunken")
				male_rb.configure(state='disabled')
				female_rb.configure(state='disabled')
				others_rb.configure(state='disabled')
				phn.configure(state='readonly')
		...
		editDetails_btn = Button(b_frame,text='Edit',bg='white',relief='sunken',fg='cyan4',width=15,font='timenewroman 14',activebackground='green',command = editDetails)
		editDetails_btn.place(x=430,y=175)
	...
	def staff():
		...
		label.image=img
		label.place(x=0,y=0)
		l = Label(b_frame,text='Profile Details will be Available soon')
		#l.place(x=180,y=0)
		'''smf4 = Frame(b_frame,height=150,width=175,bg='white')
		tc = Label(smf4,text='Total Customers:',fg='white',bg='cyan4',width=130,height=2,font='helvetica 15')
		tc.pack(side='top')
	...
	def rooms():
		...
		def roomdet(rno, add_room=False):
			# if rno is None and add_room:
			# 	Label(b_frame,text='Add New Room',font='msserif 15',fg='white',bg='cyan4',width=10).place(x=535,y=0)
			# 	l1 = Label(b_frame,text='Name :',bg='gray93',font='15').place(x=330,y=50)
			# 	name = Entry(b_frame, font='msserif 14')
			# 	name.place(x=430,y=50)
				
				
			# 	l2 = Label(b_frame,text='Email :',bg='gray93',font='15').place(x=330,y=80)
			# 	email = Entry(b_frame, font='msserif 14')
			# 	email.place(x=430,y=80)

			# 	l3 = Label(b_frame,text='Phn No :',bg='gray93',font='15').place(x=330,y=110)
			# 	phn = Entry(b_frame, font='msserif 14')
			# 	phn.place(x=430,y=110)
			...
			total_beds = Entry(smf1,fg='cyan4',bg='white',font='msserif 35')
			total_beds.insert(0,str(rdata[0][1]))
			total_beds.configure(state='readonly')
			total_beds.pack()
			...
			ac_avb = Entry(smf2,fg='cyan4',bg='white',font='msserif 35')
			ac_avb.insert(0,str(rdata[0][2]))
			ac_avb.configure(state='readonly')
			ac_avb.pack()
			...
			tv_avb = Entry(smf2,fg='cyan4',bg='white',font='msserif 35')
			tv_avb.insert(0,str(rdata[0][3]))
			tv_avb.configure(state='readonly')
			tv_avb.pack()
			...
			wifi = Entry(smf2,fg='cyan4',bg='white',font='msserif 35')
			wifi.insert(0,str(rdata[0][4]))
			wifi.configure(state='readonly')
			wifi.pack()
			...
			price = Entry(smf2,fg='cyan4',bg='white',font='msserif 35')
			price.insert(0,str(rdata[0][5]))
			price.configure(state='readonly')
			price.pack()
			...
			if rdata[0][6]=='Unreserved':
				...
				p = 'Yes'
			reserved_status = Entry(smf2,fg='cyan4',bg='white',font='msserif 35')
			reserved_status.insert(0,p)
			reserved_status.configure(state='readonly')
			reserved_status.pack()
			# Label(smf2,text=p,fg='cyan4',bg='white',font='msserif 35').pack()


			# SHOW Edit room only for super admin login
			def editRooms():
				# Make fields editable
				if editRoom_btn.config('relief')[-1] == 'sunken':
					editRoom_btn.config(text="Save")
					editRoom_btn.config(relief="raised")
					total_beds.configure(state='normal')
					ac_avb.configure(state='normal')
					tv_avb.configure(state='normal')
					wifi.configure(state='normal')
					price.configure(state='normal')
					reserved_status.configure(state='normal')
				else:
					# print(name.get(),email.get(),phn.get(),var.get(),id)
					print(total_beds.get(),ac_avb.get(),
								tv_avb.get(),wifi.get(),price.get(),reserved_status.get(),rno)
					cur.execute("update roomd set beds=?,ac=?,tv=?,internet=?,price=?,rstatus=? where rn=?",(total_beds.get(),ac_avb.get(),
								tv_avb.get(),wifi.get(),price.get(),reserved_status.get(),rno))
					con.commit()
					editRoom_btn.config(text="Edit")
					editRoom_btn.config(relief="sunken")
					total_beds.configure(state='disabled')
					ac_avb.configure(state='disabled')
					tv_avb.configure(state='disabled')
					wifi.configure(state='readonly')
					price.configure(state='readonly')
					reserved_status.configure(state='readonly')

			if isSuperAdmin:
				editRoom_btn = Button(b_frame,text='Edit',bg='white',relief='sunken',fg='cyan4',width=20,font='timenewroman 14',activebackground='green',command=lambda:editRooms())
				editRoom_btn.place(x=730,y=190)
		...
		# add_button = Button(b_frame,font='mssherif 10', text="Add Room", bg='white',fg='cyan4',width=10,command=lambda:roomdet(None, True))
		b1  = Button(b_frame,font='mssherif 10', text="Room 1", bg='white',fg='cyan4',width=10,command=lambda:roomdet(1))
		...
		# sidebuttons.window_create("end",window=add_button)
		sidebuttons.insert("end","\n")
		sidebuttons.window_create("end",window=b1)
		sidebuttons.insert("end","\n")
		sidebuttons.window_create("end",window=b2)
	...
	def bookings():
		...
		Label(b_frame,text='Booking Information',font='msserif 15',bg='gray93').place(x=225,y=0)
		cur.execute("SELECT * FROM bookings where user_id = ?",(int(login_user_id),))
		data = (row for row in cur.fetchall())
		print(data)
		table = Table(b_frame, headings=('id','f_name','l_name','phone_number','email','room_num','day', 'month', 'year', 'time', 
						'payment_method', 'user_id', 'num_of_days', 'totalamt'), rows=data)
	...
	def reserve():
		...
		Label(b_frame,text='Personal Information',font='msserif 18',bg='gray93').place(x=225,y=0)
		...
		Label(b_frame,text='Contact Information',font='msserif 18',bg='gray93').place(x=225,y=90)
		...
		Label(b_frame,text='Reservation Information',font='msserif 18',bg='gray93').place(x=225,y=175)
				...
				if temp[0] == 'Reserved':
					...
					def f():
						if pmethod != '':
							print (pmethod.get())
							print ('pmethod value')
							cur.execute("select id from bookings order by id desc")
							x = cur.fetchone()
							if x is None:
								cid = 0
							else:
								cid = int(x[0])
							cid+=1
							cur.execute("insert into bookings values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(cid,fn.get(),ln.get(),int(cn.get()),em.get(),roomn.get(),str(now.strftime("%d")),str(now.strftime("%b")),str(now.strftime("%Y")),str(now.strftime("%H:%M")),str(pmethod.get()),login_user_id,int(nod.get()),amtpd,))
		...
		# Showing unreserve option only for super admin
		if isSuperAdmin:
	...
	img3 = ImageTk.PhotoImage(Image.open(path3))
	b3 = Button(sl_frame,image=img3,text='b2',bg='white',width=180,command=profile_information)
	b3.image = img3
	b3.place(x=180*4,y=0)
	path4='images/payments.png'
	img4 = ImageTk.PhotoImage(Image.open(path4))
	b4 = Button(sl_frame,image=img4,text='b2',bg='white',width=180,command = bookings)
	b4.image = img4
	...
	Label(sl_frame,text='Profile',font='msserif 13',bg='white').place(x=774,y=106)
	Label(sl_frame,text='Bookings',font='msserif 13',bg='white').place(x=595,y=106)
...
class Table(Frame):
    def __init__(self, parent=None, headings=tuple(), rows=tuple()):
        ...
        table = ttk.Treeview(self, show="headings", selectmode="browse", height=10)
        table["columns"] = headings
        table["displaycolumns"] = headings
  
        for head in headings:
            table.heading(head, text=head, anchor=tk.CENTER)
            table.column(head, width=10, stretch=True, anchor=tk.CENTER)
        ...
        SHBar = tk.Scrollbar(self, orient = tk.HORIZONTAL)
        SHBar.pack (side = tk.BOTTOM, fill = "x")
        SHBar.config(command = table.xview)
        SVBar.config(command = table.yview)
        ...
        # scrolltablex = tk.Scrollbar(self, command=table.xview)
